...
//
// TypeScript « effaçable » exécuté par Node ≥ 22.18 (type-stripping natif), en point d'entrée
// (`node tools/cladogram.ts …`) comme à l'import. Pas de build, pas de tsconfig.
//
// CLI :  node tools/cladogram.ts <commande> [args]
...
import { deriveState, formatDerived, applySize } from "./derive.ts";
import type { AncestryMap, CladeNode } from "./derive.ts";

// ─── Types ──────────────────────────────────────────────────────────────────
/** Un nœud de l'arbre : nœud du cladogramme + `parent` (plat) + `.children` (reconstruit). */
interface TreeNode extends CladeNode {
  parent?: string;
  children?: TreeNode[];
  ref?: string;
  cd?: string;
...
interface Root { children: TreeNode[]; }
/** Données consolidées : le YAML plat + les mutations + la vue arborescente. */
interface Data {
  title?: string;
  rootNote?: string;
  backlog?: unknown;
...
interface Eco { biomes: Set<string>; habitats: Set<string>; }
...
// ---- IO ----
export function loadRaw(): { data: Data; header: string } {
  const treeText: string = fs.readFileSync(TREE_FILE, "utf8");
  ...
  const nodes: TreeNode[] = tree.nodes ?? [];
 ...
 */
export function buildRoot(nodes: TreeNode[]): Root {
  ...
  const basalFirst = (a: TreeNode, b: TreeNode) => (a.mut ? 1 : 0) - (b.mut ? 1 : 0);
...
const flowField = (k: string, v: any): string =>
...
export const emitFlat = (nodes: TreeNode[]): string =>
  "nodes:\n" + nodes.map((n) =>
    FLAT_ORDER.filter((k) => (n as any)[k] !== undefined)
      .map((k, i) => (i === 0 ? "  - " : "    ") + flowField(k, (n as any)[k]))
      .join("\n")
  ).join("\n");
...
export function emitMinimalTree(root: Root): string {
  const out: string[] = [];
  for (const c of root.children) {
    (function emit(node: TreeNode, indent: number) {
...
export const saveTree = (data: Data, header: string): void => {
  ...
  (function walk(n: Root | TreeNode) { for (const c of (n as any).children ?? []) { ordered.push(c); walk(c); } })(root);
...
export function findAny(root: Root, label: string): TreeNode | null {
  let found: TreeNode | null = null;
  (function w(n: Root | TreeNode) { if (found) return; if ((n as TreeNode).name === label || (n as TreeNode).tip === label) { found = n as TreeNode; return; } if ((n as any).children) (n as any).children.forEach(w); })(root);
  return found;
}
export function findByUid(root: Root, uid: string): TreeNode | null {
  let found: TreeNode | null = null;
  (function w(n: Root | TreeNode) { if (found) return; if ((n as TreeNode).uid === uid) { found = n as TreeNode; return; } if ((n as any).children) (n as any).children.forEach(w); })(root);
  return found;
}
// Mise à plat de l'arbre en Map<uid, { node, parent }> (parent = uid du parent, null au sommet).
export function flatten(root: Root): AncestryMap {
  const map: AncestryMap = new Map();
  (function w(n: any, parent: string | null) {
    if (n.uid) map.set(n.uid, { node: n, parent });
    if (n.children) n.children.forEach((c: TreeNode) => w(c, n.uid ?? parent));
  })(root, null);
  return map;
}
...
export const BIOME_LETTER: Record<string, string> = { north: "N", dawn: "L", dusk: "C", south: "S" };

const applyEco = (set: Set<string>, mut: any, addKey: string, removeKey: string): Set<string> => {
 ...
 */
export function deriveEcology(data: Data): Map<string, Eco> {
  const out = new Map<string, Eco>();
  (function walk(node: TreeNode, biomes: Set<string>, habitats: Set<string>) {
  ...
  })(data.root as any, new Set(data.rootBiome ?? []), new Set(data.rootHabitat ?? []));
  return out;
}

/** Lettres NLCS (ordre canonique) d'un ensemble de biomes dérivé. */
export const biomesToLetters = (set: Set<string>): string =>
...
export function validate(data: Data) {
  ...
  const used = new Set<string>(), unknown: string[] = []; let leaves = 0, nodes = 0;
  (function w(n: TreeNode) { if (n.mut) { used.add(n.mut); if (!keys.has(n.mut)) unknown.push(n.mut); } if (n.children && n.children.length) { nodes++; n.children.forEach(w); } else leaves++; })(data.root as any);
  // Structure de la liste plate : uid uniques, parent existant (ou "root").
  const byUid = new Map<string, TreeNode>(), dupUid: string[] = [], orphan: string[] = [];
  for (const n of data.nodes ?? []) { if (byUid.has(n.uid)) dupUid.push(n.uid); byUid.set(n.uid, n); }
  for (const n of data.nodes ?? []) if (n.parent !== "root" && !byUid.has(n.parent as string)) orphan.push(`${n.uid}→${n.parent}`);
  return { total: keys.size, used: used.size, unused: [...keys].filter((k) => !used.has(k)), unknown: [...new Set(unknown)], leaves, nodes, dupUid, orphan };
}

// ---- CLI ----
const req = <T>(x: T, what: string): T => { if (!x) throw new Error(`introuvable : ${what}`); return x; };
...
function printTree(): void {
  const { data } = loadRaw();
  (function w(n: TreeNode, d: number) {
 ...
 */
function ecologyReport(flag?: string): void {
  const { data } = loadRaw();
  const eco = deriveEcology(data);

  const okB = new Set(BIOMES), okH = new Set(HABITATS);
  const badKeys: string[] = [];
  for (const [key, m] of Object.entries<any>(data.mutations)) {
  ...
  const leaves: Array<{ uid: string; biomes: string; habitats: string[] }> = [], noBiome: string[] = [], noHabitat: string[] = [];
  ...
  const carriers = Object.entries<any>(data.mutations).filter(([, m]) =>
...
const editTree = (fn: (d: Data) => void): void => { const { data, header } = loadRaw(); fn(data); saveTree(data, header); report(); };
...
const mutKeyRe = (key: string): RegExp => new RegExp("^  " + reEsc(key) + ":\\s*$");
const isMutKey = (l: string): boolean => /^  [A-Za-z][\w]*:\s*$/.test(l);
const eolOf = (text: string): string => (text.includes("\r\n") ? "\r\n" : "\n");
function patchMutScalar(key: string, section: string, value: string): void {
...
function addMutation(key: string, label: string, desc: string): void {
  const text = fs.readFileSync(MUT_FILE, "utf8");
  const eol = eolOf(text);
...
function deriveCmd(...uids: string[]): void {
...
const CMDS: Record<string, (...args: any[]) => void> = {
  ...
  "mut-add": (key: string, label: string, desc: string) => { addMutation(key, label, desc); report(); },
  "mut-relabel": (key: string, label: string) => { patchMutScalar(key, "label", label); report(); },
  "mut-describe": (key: string, desc: string) => { patchMutScalar(key, "description", desc); report(); },
  "node-mut": (name: string, key: string) => editTree((d) => { if (!d.mutations[key]) throw new Error(`clé inconnue : ${key}`); req(findAny(d.root, name), name)!.mut = key; }),
  "node-clear-mut": (name: string) => editTree((d) => { delete req(findAny(d.root, name), name)!.mut; }),
  "node-cd": (name: string, cd: string) => editTree((d) => { req(findAny(d.root, name), name)!.cd = cd; }),
  "node-ref": (name: string, ref: string) => editTree((d) => { req(findAny(d.root, name), name)!.ref = ref; }),
  "node-rename": (oldN: string, newN: string) => editTree((d) => { req(findAny(d.root, oldN), oldN)!.name = newN; }),
};
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  const [cmd, ...args] = process.argv.slice(2);
  if (!cmd || !CMDS[cmd]) { console.log("Commandes : " + Object.keys(CMDS).join(", ")); process.exit(cmd ? 1 : 0); }
  try { CMDS[cmd](...args); } catch (e) { console.error("Erreur :", (e as Error).message); process.exit(1); }
}
